...
INTENT_PATTERNS = {
    ...
    ResearchIntent.COMPARISON: {
        "keywords": ["compare", "difference", "versus", "vs", "better than", 
                    "advantages", "pros cons", "which", "between"],
        "priority": 7,
        "conflicts_with": []
    },
    ResearchIntent.DEEP_RESEARCH: {
        "keywords": ["research", "comprehensive", "everything about", "deep dive", 
                    "thorough", "complete", "exhaustive", "in-depth"],
        "priority": 6,
        "conflicts_with": [ResearchIntent.QUICK_ANSWER]
    },
    ResearchIntent.DISCOVERY: {
        "keywords": ["related", "similar", "alternatives", "also", "other", 
                    "like", "find more", "explore"],
        "priority": 5,
        "conflicts_with": []
    },
    ResearchIntent.MONITORING: {
        "keywords": ["changed", "updated", "new version", "check", "monitor", 
                    "track", "latest", "recent changes"],
        "priority": 9,
        "conflicts_with": []
    },
    ResearchIntent.VALIDATION: {
        "keywords": ["check if", "verify", "validate", "exists", "accessible", 
                    "working", "available"],
        "priority": 7,
        "conflicts_with": []
    },
    ...
    # DOCUMENTATION workflow
        ...
        if context.has_urls():
            ...
            workflow.append(WorkflowStep(
                ...
                required_inputs={"root_url": context.known_urls[0], "max_pages": 25}
            ))
        else:
            workflow.append(WorkflowStep(
                tool="search_web",
...
def suggest_tools(
    ...
    for i, step in enumerate(workflow):
        suggested_params = suggest_parameters(
            step.tool, 
